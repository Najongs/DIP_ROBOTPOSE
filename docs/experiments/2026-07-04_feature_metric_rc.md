...
실루엣-단독 +0.0126 vs 실루엣+특징 +0.0178 = 특징 +0.005(가림으로 실루엣 열화 시 구조신호가 유일하게 기여하는 곳, 단 극한꼬리에서만). DINOv3 iter당 forward 비용 대비 배포 가치 없음. 40%는 상류 θ 붕괴가 지배 → RC로 못 고침(멀티스타트 반증과 일치). **40%의 정답은 가림-증강 head(T1/T2)**.
