...
## 4. 실행 랭킹 (기존 자산으로 검증 비용 순)

| 순위 | 아이디어 | 비용 | 기대 EV | 근거 |
|---|---|---|---|---|
| 1 | 3.4 가림-로버스트 실루엣 | 반나절 | 중 | Phase 4 진행 트랙의 직접 보강, orb 개선 여지 |
| 2 | 3.1 공분산 가중 PnP | 반나절 | 중 | 재학습 없음, 가림 빈 직접 타격, 기존 IRLS 확장 |
| 3 | 3.3 masked-state prior | 1일 | 중(불확실) | prior_w 훅 존재, DREAM 분포 정보량이 관건 |
| 4 | 3.2 가시성 헤드 | 2-3일(재학습) | 낮-중 | confident-wrong 4-9% 잔여분이 상한 |
| 5 | 3.6 시간 prior | 1일 | 벤치마크 외 | 프로토콜 별도 표기 필요 |
