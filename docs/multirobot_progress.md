...
## FR3 temporal tracking 미티게이션 (2026-07-07) — 재수집 없이 0.33→0.47

FR3는 **연속 촬영 궤적**(카메라스트림당 median 2.4°/frame, 99.9%가 솔버 basin 30° 이내)이라, 각 프레임 솔브를 **직전 프레임 해로 init**하면 올바른 basin을 유지 → 단안 모호성을 시간 연속성으로 우회. `Eval/fr3_track_eval.py`(스트림별 detector 배치추론 후 순차 solve). cross-session(fr3_val):

| seed | angle MAE | ADD-AUC | median |
|---|---|---|---|
| baseline (tracking 없음) | 38° | 0.33 | — |
| **head-seed (배포가능)** | 25° | **0.47** | 47mm |
| multistart-seed | 33° | 0.41 | 57mm |
| GT-seed (천장) | 15° | 0.56 | 31mm |

**결론**: 시간 tracking은 **재수집 없이 배포 가능한 소프트웨어 해법**으로 ADD-AUC를 0.33→0.47로 유의미하게 올림(팔 관절 J0-J2는 45→10°로 잘 복원). 다만 6-DOF(0.86/0.90)엔 못 미침 — 두 한계: (1) **seeding**: head가 일부 스트림을 오답 basin으로 seed(GT-seed 천장 0.56이 증거), (2) **손목 drift**: J4/J5 관측성 약해 궤적 따라 누적 오차(GT-seed로도 25-29°). 스트림들이 공통 home 포즈로 시작 안 함(frame0 J2/J4/J5 std 37-51°)이라 home-seed 불가.
- **개선 여지**: multi-hypothesis tracking(여러 seed에서 tracking 후 누적 reproj 최소 궤적 선택)로 seeding을 0.56 천장까지, 손목은 head prior anchoring으로 drift 억제. → 0.5~0.6대 배포 가능 예상. 완전 0.86 격차는 재수집(depth/동기 멀티뷰) 필요.

### multi-hypothesis + wrist anchoring 시도 (2026-07-07) — 천장 도달 실패

`Eval/fr3_track_mh.py`: K=12 hypothesis(head+random seed)를 스트림 전체에 배치 병렬 추적 → 누적 reproj 최소 궤적 선택. + `anchor_init_w`로 직전프레임 앵커링(손목 drift 억제). 16 스트림 cross-session:

| 방법 | ADD-AUC | angle |
|---|---|---|
| 단일 head-seed tracking | 0.45 | 28° |
| multi-hyp K=12 (anchor 0) | 0.456 | 32° |
| multi-hyp K=12 anchor=0.1 | **0.04** | 60° (붕괴) |

**부정 결과**: multi-hyp가 **개선 없음**(0.456≈단일). 원인 — 궤적 선택 기준이 **누적 reprojection인데 그게 바로 FR3에서 모호한 값**(오답 관절config가 저-reproj) → min-reproj가 저-reproj 오답 궤적 선택(J4/J5 오히려 악화). anchor=0.1은 균일 앵커가 관측가능 관절까지 얼려 붕괴(0.04); 작은 값도 균일 앵커 특성상 무효.
- **교훈**: reprojection 기반 selection/optimization은 FR3 모호성을 못 이김(GT-seed 같은 **외부 truth 앵커만** 통함). 배포 가능 최선 = **단일 head-seed tracking ~0.47**. 천장 0.56은 reprojection으로 얻을 수 없는 좋은 seed 필요 → **결국 재수집(depth/동기 멀티뷰)이 완전 해법.**
- FR3 최종: baseline 0.33 → temporal tracking 0.47(배포가능, 재수집 불요) → 천장 0.56(외부 seed) → 완전 해결은 재수집.
