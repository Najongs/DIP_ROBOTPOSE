...
## ✅ C8 임계 실험 결과 (2026-07-21 완료, A6000 ~3.7h GPU 공유 실행)

| cam | zero-adapt base | +RC | 배포 | RoboPEPP | vs RoboPEPP |
|---|---|---|---|---|---|
| realsense | 0.7321 | **0.8041** | 0.8153 | 0.805 | 동률 (−0.001) |
| kinect | 0.7506 | **0.8164** | 0.8275 | 0.785 | **+0.031** |
| orb | 0.7107 | **0.7556** | 0.7784 | 0.775 | −0.019 |
| azure (배포=zero-adapt, RC off) | — | **0.7945** | 0.7945 | 0.753 | **+0.042** |
| **mean** | | **0.7927** | 0.8039 | 0.780 | **+0.013** |

**판정**: 예측 범위(0.78~0.79)의 상단. 실측 적응을 전혀 쓰지 않아도 **mean 0.793으로 RoboPEPP 0.780을 명목상 상회** — 단 +0.013은 노이즈(~0.010)와 비슷한 규모라 논문에는 "동률-이상(보수적)"으로 표기. 자가학습 총기여 = **+0.011**(기존 −0.010은 angle 성분만이었으나 총량도 유사). **RC 이득은 zero-adapt에서 오히려 증가**(+0.072/+0.066/+0.045 vs 배포 +0.070/+0.062/+0.040) — 합성 헤드의 약점을 RC가 보상, "RC=엔진" 서사 강화. → C1/C8은 **소프트 해소**: "적응 없이도 baseline 이상 유지, 자가학습은 추가 마진"으로 서사 확정(강한 "무적응 SOTA" 주장은 CI 확보 전까지 보류). 표 6에 행 추가 + §4.5 "실측 적응 공정성" 문단 반영 완료.

## 잔여 작업

- [x] **C8 결과 반영** — 표 6 zero-real-adaptation 행 + §4.5 공정성 문단 (2026-07-21)
- [ ] **프레임-부트스트랩 CI**: `rc_refine_from_dump.py`가 프레임별 ADD를 저장하지 않음 → `--dump-adds` 저장 추가 후 재실행 필요 (P1로 격하; 또는 3-시드 std로 대체)
...
- [x] C8 결과에 따른 서사 확정 — "동률-이상(보수적)" 프레임 채택, 기여 목록은 3개 압축본 유지 (2026-07-21)
