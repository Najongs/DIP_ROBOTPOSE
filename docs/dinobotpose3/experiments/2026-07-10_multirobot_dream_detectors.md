# 멀티로봇 DREAM 포즈 파이프라인 — KUKA iiwa7 / Baxter-left (2026-07-09~13)

> 목적: Panda로 SOTA(real 0.804)를 낸 DINObotPose3 파이프라인 전체(검출기→angle/rot head→FK→솔버)가
> DREAM의 다른 로봇(**KUKA iiwa7**, **Baxter 좌완**)으로 일반화되는지.
> **결과**: 검출기 2D AUC KUKA 0.735 / Baxter 0.817. **포즈 ADD-AUC@100mm KUKA 0.34 / Baxter 0.25**(synth, `--direct-pose`).
> 파이프라인은 일반화됨 ✅. 천장은 로봇별 상이 — KUKA=rot-head t, Baxter=wrist 각도(관측성 천장). render-and-compare(A)는 양쪽 막힘(KUKA 메쉬 부재 / Baxter RC 레버 불일치).
> ⚠️ KUKA/Baxter는 synth·RC 없음 → Panda real과 직접 비교 불가. 전체 요약표는 아래 **포즈 성적 종합** 참조.
>
> 문서 흐름: 검출기(§결과) → KUKA 포즈(FK·head·솔버·0.34) → Baxter 포즈(0.25) → **포즈 성적 종합** → RC 조사 → wrist 진단 → DREAM baseline → 남은 일.
...
### Baxter 포즈 파이프라인 + 최종 ADD (2026-07-13, main f974c4e)
- ✅ **baxter-left FK** — 40-start scipy 피팅(single-start는 local minima에 빠짐), left_s0..w2 재현 **0.003mm**(train/held-out/test). `Eval/baxter_fk_fit.py`, `model_v4.baxter_left_forward_kinematics`.
- ✅ 배선(`--fk-robot baxter`) + `run_train_baxter_{angle,rotation}.sh` + `Eval/baxter_add_eval.py`. 스모크 통과.
- angle+rotation head 학습(GPU0/1, Ep11 중단). **최종 ADD-AUC 0.25**(mean 87mm/median 76mm, full 5982 test, `--direct-pose`). rot-head 우수(geo 5.7°/t-err 60mm, KUKA보다 좋음)이나 **wrist 각도(w0/w1) 병목**(25°/21° — 근위 J0-3은 4.5~10.6° 양호). KUKA(근위 혼동)와 **정반대 약점**.

## 포즈 성적 종합 (2026-07-13)

| 로봇 | 검출기 2D AUC (synth) | 포즈 ADD-AUC@100mm | 병목 |
|---|---|---|---|
| **Panda** (real, 배포 SOTA) | — | **0.804** (real 4-split) | — |
| **KUKA iiwa7** (synth) | 0.735 | **0.34** (med 64mm) | rot-head t (56mm) |
| **Baxter 좌완** (synth) | 0.817 | **0.25** (med 76mm) | wrist 각도 (25°) |

> ⚠️ KUKA/Baxter는 **synth test·RC 없음** → Panda real 0.804와 **직접 비교 불가**. 의의 = **DINObotPose3 파이프라인(검출기→angle/rot head→data-fit FK→솔버)이 3개 로봇으로 일반화**됨을 입증. 공통 채택 모드 = `--direct-pose`(head각도+rot-head R,t 직접, 2D 우회 = 검출기 link-혼동 회피).

---

## render-and-compare(A) 조사 — 양쪽 막힘 (2026-07-13)

포즈 천장(KUKA rot-head t, Baxter wrist)을 넘을 레버로 mesh render-and-compare 깊이보정을 시도. **메쉬 소싱** 단계별:

| 소스 | 결과 |
|---|---|
| DREAM repo | 키포인트 방법이라 **메쉬 없음** (`manip_configs`엔 keypoint 스펙만) |
| RoboPose repo | `--robot=kuka` 다운로드 명령 있으나 **rclone 공유드라이브 죽음**(2021 토큰; `about`은 인증되나 폴더 empty) |
| bullet3 `data/kuka_iiwa` | link_*.obj 있으나 **DREAM kuka와 ~20mm 다른 iiwa7 변종**(rpy 구조까지 상이, hybrid도 21mm) → 정렬 불가 |
| baxter_common `baxter_description` | S0~W2.DAE + URDF = **DREAM baxter와 FK 0.00mm 동일** ✅ (오프셋 [0.279,0.102,0.271,...]까지 일치) |

- **KUKA RC**: 정확한 메쉬 없음 → **차단**. 메쉬-불필요 cov-PnP(FK(head각도) vs 검출2D)도 **link-혼동으로 발산**(median 1074mm).
- **Baxter RC**: 메쉬·URDF FK(full SE(3))·nvdiffrast 실루엣·SAM 마스크 전부 작동(**SAM vs GT-render IoU 0.82**, 팔 정합 시각 확인). 그러나 **실루엣 RC가 포즈 악화**(ADD 77→204mm t-only / →253mm t+각도): (a) 실루엣-깊이 모호성으로 자유 최적화가 표류, (b) Baxter 병목이 깊이가 아니라 **wrist 각도**라 포즈/깊이 보정기로 못 고침. Panda식 앵커링+do-no-harm 게이팅 없이는 해로움 → **RC는 Baxter에 레버 불일치**.
- 코드: `Eval/baxter_{render,rc_eval}.py`, 에셋 `ViS/Baxter/`(메쉬 git 추적), 소스 clone `_assets_src/`(bullet3, baxter_common).

## wrist 병목 진단 — 관측성 천장 (2026-07-13)

RC가 wrist를 못 고치므로 wrist 자체를 파고듦:
- **GT 2D를 angle head에 주입해도 wrist 거의 안 개선**(w0 28.1→27.6°, w1 22.8→22.2°; `model_angle.forward(kp2d_override=...)`). → **검출 문제가 아님**.
- 원인 = **관측성 천장**: wrist 관절의 자기축 회전은 자기 키포인트 원점을 안 움직여, 완벽한 키포인트로도 각도 미결정. (메모리 [[meca-wrist-observability-ceiling]]와 동일 — Meca도 같은 벽)
- 시도한 해법 = 기하 불가 → **appearance로 wrist-roll을 읽기**. `mlp_patch` head(end-effector k×k 특징 패치)가 정확히 이를 위해 존재(독스트링: "단일 샘플은 gripper orientation을 놓침"). Baxter head는 plain `mlp`라 이걸 못 봤음. Meca는 appearance head로 ADD 0.90 돌파한 선례.
- ❌ **반증(2026-07-13)**: mlp_patch **k=3**·**k=5**(`KP_PATCH_K` env) 재학습 9에폭 관찰 결과, 두 변종 모두 wrist(w0/w1)가 plain mlp를 **못 이김**(Ep8 k=3 wrist 29.8°/23.8° vs plain 27.9°/22.6°; k=5는 plain과 동률). 즉 **end-effector appearance 패치로도 관측성 천장이 안 풀린다.** DR 저해상도 크롭에서 손목-롤이 그리퍼 외형만으로 신뢰성 있게 읽히지 않는 것으로 보임. → Baxter wrist는 이 도구셋으로는 근본 한계. (Meca가 됐던 건 로봇/데이터 특성 차이로 추정 — 추후 대조 필요.) 두 런 중단, GPU 회수.

## DREAM 검출기 baseline (참고, 2026-07-13)

- DREAM 공식 검출기 gdown 다운로드(`DREAM/trained_models/`): `kuka_dream_resnet_h`, `baxter_dream_vgg_q`.
- DREAM KUKA 검출기를 우리 synth test에 추론(`DREAM/scripts/network_inference_dataset.py`): keypoint **AUC 0.756**(L2 median 3.4px) — **우리 검출기 0.735와 동급**. 단 DREAM은 known-angle PnP라 포즈 프로토콜이 달라 포즈 AUC는 비교 불가.

## 남은 일

1. ✅ **FK** — KUKA/Baxter 모두 data-fit 0.003mm.
2. ✅ **head 배선 + 솔버 연결·측정** — KUKA 0.34 / Baxter 0.25 (`--direct-pose`).
3. ❌ **Baxter wrist appearance(mlp_patch) — 반증**. k=3/k=5 모두 plain mlp를 못 이김(관측성 천장 미해결). 이 도구셋으론 Baxter wrist가 근본 한계.
4. **KUKA 포즈**: rot-head t(56mm)가 천장. mesh RC는 정확한 kuka 메쉬 부재로 차단(외부 확보 필요).
5. **real 평가셋 없음** — DREAM은 kuka/baxter가 synth-only(공개 real 데이터 자체가 없음) → real SOTA 비교 불가는 도구 한계가 아니라 데이터 본질.
