...
- 해법 = 기하 불가 → **appearance로 wrist-roll을 읽기**. `mlp_patch` head(end-effector k×k 특징 패치)가 정확히 이를 위해 존재(독스트링: "단일 샘플은 gripper orientation을 놓침"). Baxter head는 plain `mlp`라 이걸 못 봤음. Meca는 appearance head로 ADD 0.90 돌파.
- 🔄 **재학습 진행 중**: mlp_patch **k=3**(GPU0) + **k=5**(GPU1, `KP_PATCH_K` env). **관전: wrist가 plain 수렴치(25°/21°) 아래로 내려가는가.** 완료 시 auto-eval로 `--direct-pose --head-type mlp_patch` ADD 재측정.
...
3. 🔄 **Baxter wrist appearance(mlp_patch) 재학습** — 진행 중. 관측성 천장을 appearance로 돌파 시도(k=3/k=5).
