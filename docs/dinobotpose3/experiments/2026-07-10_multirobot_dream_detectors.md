...
### render-and-compare(A) 조사 결과 — 양쪽 다 막힘 (2026-07-13)
- **KUKA RC**: iiwa7/Allegro 메쉬가 DREAM·RoboPose(드라이브 죽음)에 없음. bullet3 kuka 메쉬는 있으나 **DREAM kuka와 ~20mm 다른 변종**(rpy 구조까지 상이) → 정렬 불가.
- **Baxter RC**: baxter_common URDF+메쉬 = **DREAM baxter와 0.00mm 동일** ✅. 메쉬·FK·nvdiffrast 실루엣·SAM 마스크 전부 작동(SAM vs GT-render IoU **0.82**). 그러나 **실루엣 RC가 포즈를 악화**(ADD 77→204mm t-only, →253mm articulated): (a) 실루엣-깊이 모호성으로 자유 최적화가 표류, (b) Baxter 병목이 깊이가 아니라 **wrist 각도**(25°, 검출기 원위 link-혼동)라 포즈/깊이 보정기로 안 고쳐짐. Panda식 앵커링+do-no-harm 게이팅 없이는 해로움. → **Baxter도 RC는 맞는 레버 아님**(병목=wrist 각도). 코드: `Eval/baxter_{render,rc_eval}.py`, 에셋 `ViS/Baxter/`.
- **결론**: KUKA/Baxter 포즈 천장은 각각 **0.34 / 0.25 (direct-pose)** 유지. 개선하려면 RC가 아니라 wrist/원위 키포인트 검출 개선이 방향.

## 남은 일

1. ✅ **iiwa7 FK** — 완료·검증(0.003mm). Baxter FK도 완료(multi-start, 0.003mm).
...
- angle+rotation head 학습(GPU0/1, Ep11에서 중단). **최종 ADD-AUC 0.25**(mean 87mm/median 76mm, full 5982 test, `--direct-pose`). rot-head 우수(geo 5.7°/t-err 60mm, KUKA보다 좋음)이나 **wrist 각도(w0/w1) 병목**(J4/J5 25°/21° — 검출기 원위 link-혼동이 각도 head로 전파; 근위 J0-3은 4.5~10.6°로 양호). KUKA(근위 혼동)와 **정반대 약점**. 더 돌리면 wrist가 천천히 개선되나 rot-head는 이미 수렴 — 각도 wrist가 천장.
