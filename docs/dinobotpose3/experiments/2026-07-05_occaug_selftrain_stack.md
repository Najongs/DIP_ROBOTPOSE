...
**참고 (정확도만 보면)**: 개선 스택 배포 +RC = **0.8254** (기존 0.8213 +0.004, light-스택 +0.009로 최고) — "배포 head self-train 2라운드"가 정확도는 올림. 하지만 가림 강건성 없음(40% 0.378≈base). 강건성 목표엔 light-스택, 순수 정확도 목표엔 개선 스택(0.8254) — 목표별 선택. 사용자 요청=강건성 → light-스택.
