...
이에 더해 DREAM이 포함하는 세 로봇(Panda, KUKA iiwa7, Baxter) 전부에 동일 파이프라인을 적용하여, 적용 가능성과 로봇별 병목(예: 손목 관절의 관측성 한계)을 분석한다(§4.7).
...
> EN: In addition, we apply the same pipeline to all three robots covered by DREAM (Panda, KUKA iiwa7, and Baxter), analyzing its applicability and the per-robot bottlenecks, such as an observability limit at the wrist joints (§4.7).
