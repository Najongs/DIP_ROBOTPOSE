...
우리 설계는 두 축으로 요약된다. **인식 축**: 기존 방법들이 인코더를 학습·적응시키는 것과 달리 백본을 완전히 동결한다 — 편의가 아니라 실험적 선택이다. 백본을 실측에 적응시키는 시도들이 일관되게 포즈 정확도를 악화시켰고, 적응은 거친 2D 지표(PCK)를 올리면서도 기하 솔버가 요구하는 서브픽셀 정밀도를 파괴한다(§4.9). **기하 축**: 2D 키포인트가 원리적으로 못 주는 깊이 제약을 학습된 회귀가 아니라 테스트-타임 렌더-비교로 복원하며, 이것이 단일 최대 성능 레버다(ΔMean +0.043, §4.4). 이 두 축 위에서 DREAM-Panda 실측 평균 ADD-AUC 0.804(RoboPEPP 대비 +0.024, 실행-간 노이즈의 2배 이상)를 달성하고, 어느 카메라에서도 뒤지지 않으며, 평가한 전(全) 가림 수준(0–40%)에서도 앞선다.

> EN: Our design reduces to two axes. **Perception**: unlike prior methods that train or adapt their encoders, we keep the backbone fully frozen — an experimental choice, not a convenience: attempts to adapt the backbone to real images consistently degraded pose accuracy, with adaptation raising coarse 2D metrics (PCK) while destroying the sub-pixel precision the geometric solver needs (§4.9). **Geometry**: the depth constraint that 2D keypoints fundamentally lack is restored not by a learned regressor but by test-time render-and-compare — the single largest performance lever (ΔMean +0.043, §4.4). On these two axes we reach a mean ADD-AUC of 0.804 on DREAM-Panda real (+0.024 over RoboPEPP, more than twice the run-to-run noise), trail on no camera, and lead across all evaluated occlusion levels (0–40%).

**기여(Contributions).**
1. **Predicted-joint + 자동 bbox 최고 성능**: DREAM-Panda 실측 4-스플릿 평균 ADD-AUC 0.804로 RoboPEPP(0.780)·RoboTAG(0.740)를 능가한다. 이들은 우리와 **동일한 자동-bbox 프로토콜**이라 공정한 동일-조건 비교이며(반면 HoRoPose는 GT-bbox를 쓰고 자동 검출기에선 붕괴, §4.5), 우리는 완전 자동 bbox로 SOTA다.
2. **동결 파운데이션 프론트엔드는 제약이 아니라 발견이다**: 깨끗하게 평가된 두 백본-적응 방식(공격적 SSL·pseudo-keypoint co-finetune)이 모두 ADD를 악화시킨다(§4.9; 온건 SSL은 헤드 비정합으로 평가 불능). 특히 공격적 SSL은 real PCK@5를 +0.069 올리면서도 ADD를 0.567→0.531로 떨어뜨린다 — 적응은 거친 2D 강건성을 얻는 대가로 기하 솔버가 요구하는 서브픽셀 정밀도를 판다(키포인트-노이즈 민감도와 정량 정합, §4.4). 성능은 특정 백본이 아니라 파운데이션 특징 일반에서 오며(§4.10, DINOv3≈SigLIP2), DINOv3 채택 근거는 동결 체제의 검출 우위(0.80 vs 0.72)다.
...
> EN: **Table 6. Leave-one-out ablation (locked 1000, ADD-AUC@100mm);** each row removes one lever from the deployed Full model. †azure ships RC off, so −RC leaves azure unchanged. ‡This row reverts only the **angle**-side occ-aug/self-training; the per-camera self-trained **rotation** heads are retained on rs/kinect/orb — the −0.010 therefore under-states the total real-adaptation contribution, which is isolated by the zero-real-adaptation ablation (in progress).

> ‡주의: 이 행은 **angle 헤드**의 occ-aug/자가학습만 제거하며, rs/kinect/orb의 카메라별 자가학습 **rot 헤드는 유지**된다 — 따라서 −0.010은 실측 적응의 총기여를 과소평가하며, 총기여는 zero-real-adaptation 절제(측정 중)로 별도 산출한다.
...
**관측성 천장 분석(그림 8).** Baxter 손목의 지배적 오차는 검출 실패가 아니라 **관측성**에서 온다: 손목 관절의 자기축 회전은 자기 키포인트 원점을 움직이지 않아, 완벽한(GT) 키포인트를 헤드에 주입해도 손목 각도가 거의 개선되지 않는다(손목 MAE 28.1°→27.6°). 즉 2D 키포인트 기하만으로는 손목 방향이 원리적으로 미결정이다. 남은 후보는 그리퍼/손목의 **appearance**를 읽는 것이나, 엔드이펙터 특징 패치를 명시적으로 넣은 헤드(크기 3·5)도 표준 헤드의 손목 정확도를 넘지 못했다 — 저해상도 도메인-랜덤화 크롭에서는 손목-롤이 외형만으로 신뢰성 있게 결정되지 않음을 시사한다. 이는 원위 관절의 관측성 한계가 기하뿐 아니라 appearance 단서로도 쉽게 깨지지 않는, 견고한 천장임을 보여준다.

> EN: **Observability-ceiling analysis (Fig. 8).** Baxter's dominant wrist error comes from **observability**, not detection failure: a wrist joint's self-axis rotation does not move its own keypoint origin, so injecting perfect (GT) keypoints into the head barely changes the wrist angle (wrist MAE 28.1°→27.6°). The wrist orientation is thus fundamentally under-determined by 2D keypoint geometry. The remaining candidate is to read the **appearance** of the gripper/wrist, but a head with an explicit end-effector feature patch (sizes 3 and 5) did not surpass the standard head on wrist accuracy either — suggesting that wrist roll is not reliably determined from appearance in low-resolution domain-randomized crops. The distal-joint observability ceiling is thus a robust one, not easily broken by geometry *or* appearance cues.
