...
> **render-compare를 쓰는가?** — RoboPose(학습된 반복 R&C) · CtRNet/CtRNet-X(학습-타임 실루엣 자기지도) · **우리**(테스트-타임 SAM-실루엣)만 쓴다. **RoboPEPP · HoRoPose · RoboKeyGen은 render-compare를 쓰지 않는다**(각각 feed-forward+PnP / DepthNet / diffusion). 즉 우리 주 baseline RoboPEPP과 비교하면 **render-compare 자체가 차별점**이다.
