...
AOUT=./outputs_fr3/angle_clean_$(date +%Y%m%d_%H%M%S); mkdir -p "$AOUT"
python3 train_angle.py \
  ...
  --head-type mlp --epochs 40 --batch-size 16 --lr 1e-3 --num-workers 10 \
  --wandb-project fr3-angle-clean > "$AOUT/train.log" 2>&1
echo "$AOUT" > outputs_fr3/ANGLE2_DONE
